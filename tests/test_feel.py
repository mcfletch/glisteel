...
class TestWhatATapAtSpeedDoes:
    ...
    @pytest.mark.xfail(strict=True, reason=(
        "the wheel centres and the car keeps the direction the tap left it "
        "pointing, as a real one does -- so a driver who never corrects still "
        "runs out of road, six seconds later instead of one. Whether the game "
        "should straighten the car for a player who cannot meter a "
        "counter-tap is section 4.10."))
...
class TestBeingMired:
    ...
    LINE = 'throttle 0..14; left 6..6.4'

    def test_a_run_ends_when_the_car_stays_off_the_road(self) -> None:
        trace = _drive(self.LINE, seconds=14.0)
    ...
    def _after_it_ended(self, seconds=14.0):
        ...
        trace = _drive(self.LINE, seconds=seconds)
