...
class TestTheReadouts:
    ...
    def test_off_the_track_says_so_and_goes_quiet_again(self) -> None:
        hud = RaceHUD()
        hud.show(speed_kph=0.0, off=True)
        assert str(hud.warning.value) == 'OFF TRACK'
        hud.show(speed_kph=0.0, off=False)
